...
package zxyilian.myflipfragmentdemo.flip;
...
import zxyilian.myflipfragmentdemo.R;
...
public class TitleIndicator extends LinearLayout implements View.OnClickListener,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        path.moveTo(left_x, top_y+5f );//这里是1dp的误差，应该用dimens写，不然不同屏幕怕还是有误差
        path.lineTo(right_x, top_y+5f  );
        path.lineTo(right_x, bottom_y + 5f);
        path.lineTo(left_x, bottom_y + 5f);
        path.close();
